...
protected:
    ...
    void deleteNode(Node<Key, Value>* curr);
...
template<class Key, class Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::search(Node<Key, Value>* node, const Key& k) const{
...
template<class Key, class Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::insertInternal(Node<Key, Value>* node, const std::pair<const Key, Value> &keyValuePair) const{
    if (!node) {
...
int BinarySearchTree<Key, Value>::height(Node<Key, Value>* root) {
	...
	int curr = ((left > right) ? left : right) + 1;
	return curr;
}

template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::isBalancedInternal(Node<Key, Value>* node){
...
{
    deleteNode(this->root_);
}

/**
* A helper function to delete all the children nodes and the node.
*/
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::deleteNode(Node<Key, Value>* curr){
    if(curr ){
        deleteNode(curr->getLeft());
        // deleteNode(curr->getRight());
        free(curr);
    }
}
